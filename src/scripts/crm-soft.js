...
const popup = overlay ? overlay.querySelector('.call-popup') : null;
const closeBtn = overlay ? overlay.querySelector('.call-popup__close') : null;
...
tablePopupCloseBtn?.addEventListener('click', () => {
  ...
  if (tablePopupContent) tablePopupContent.innerHTML = '';
})
let popupData = null;
...
function initPage() {
  if (urlCode) {
    if (pageMkb) pageMkb.classList.remove('page__mkb--start');
    const searchInput = document.getElementById('search-input');
    if (searchInput) searchInput.codeValue = urlCode;
    hideMkbData();
    searchMkb();
